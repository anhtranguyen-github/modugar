...
class VectorStoreManager:
    def __init__(self):
        ...
        self.active_store_name: str = 'Weaviate'  # Default to Weaviate
    
    def get_available_stores(self) -> list[str]:
        """Get list of available vector store implementations"""
        return list(self.stores.keys())
    
    async def initialize_store(self, store_name: str, **kwargs) -> Any:
        """Initialize and connect to a vector store"""
        if store_name not in self.stores:
            raise ValueError(f"Vector store '{store_name}' not found")
        
        store = self.stores[store_name]
        client = await store.connect(**kwargs)
        if client:
            self.active_store = store
            self.active_store_name = store_name
            return client
        return None
    ...
    async def create_collection(self, collection_name: str, **kwargs) -> bool:
        """Create a new collection in the active vector store"""
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.create_collection(collection_name, **kwargs)
    
    async def delete_collection(self, collection_name: str) -> bool:
        """Delete a collection from the active vector store"""
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.delete_collection(collection_name)
    
    async def insert_vectors(self, collection_name: str, vectors: list[list[float]], metadata: list[dict], **kwargs) -> list[str]:
        """Insert vectors into the active vector store"""
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.insert_vectors(collection_name, vectors, metadata, **kwargs)
    ...
    async def search_vectors(self, collection_name: str, query_vector: list[float], limit: int, **kwargs) -> list[dict]:
        ...
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.search_vectors(collection_name, query_vector, limit, **kwargs)
    
    async def delete_vectors(self, collection_name: str, vector_ids: list[str]) -> bool:
        """Delete vectors from the active vector store"""
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.delete_vectors(collection_name, vector_ids)
    
    async def get_collection_info(self, collection_name: str) -> dict:
        """Get information about a collection in the active vector store"""
        if not self.active_store:
            raise Exception("No active vector store")
        return await self.active_store.get_collection_info(collection_name)
