...
def create_test_pdf(path):
    ...
    c.drawString(100, 730, "This is a test document for the RAG pipeline with Weaviate.")
...
async def test_rag_pipeline_weaviate():
    """Test the RAG pipeline with Weaviate vector store"""
    # Initialize managers
    ...
    file_config = FileConfig(
        ...
        labels=["test", "pdf"],
        source="local",
        metadata="Test document for RAG pipeline with Weaviate",
        fileID="test_001",
        isURL=False,
        overwrite=True,
        rag_config={
    ...
    try:
        ...
        print("\n4. Storing in Weaviate...")

        # Initialize Weaviate connection
        client = await vector_store_manager.initialize_store(
            "Docker",  # Use Docker configuration
            url="weaviate",  # Use container name
        ...
        vector_store_config = (
            file_config.rag_config["VectorStore"].components["Weaviate"].config
        )
        ...
        print(f"Successfully stored {len(vector_ids)} vectors in Weaviate")
...
if __name__ == "__main__":
    try:
        ...
        asyncio.run(test_rag_pipeline_weaviate())
